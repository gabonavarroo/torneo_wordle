"""
Híbrido RG3 — refina RG2 con mejor fallback uniforme y ajuste de umbrales.

Cambios vs RG2:
- Para uniform, cuando hay muchos candidatos (n > 6) el fallback usa
  sondas dinámicas (vocab + no-palabras) en lugar de solo entropía,
  buscando particiones más agresivas en 4L/5L donde quedaban fallos.
- En frequency, el umbral de adivinar directo en T3 baja a 0.60 para
  aprovechar probabilidades altas bajo shock y reducir mean.
- Mantiene tablas T2/T3 desde el root y la lógica de safe-guess tardía.
"""
...
# Direct en T3: en frequency bajamos a 0.60 para aprovechar p_best altas
...
def _choose_t4(candidates: Sequence[str], vocab: Sequence[str], wl: int, mode: str,
    ...
    if p_best > T4_DIRECT_PROB_THRESHOLD_UNI:
        return _most_probable(candidates, probs)
    # Uniform: con n grande usar sondas, con n pequeño entropía pura
    if n > 6:
        return _dynamic_best(candidates, vocab, wl, probs, max_pool=450, n_probes=15)
...
class RG3_gabriel_regina(Strategy):
    ...
    def begin_game(self, config: GameConfig) -> None:
        ...
        RG3_gabriel_regina._load_tables(self._wl, self._mode)
        self._t2 = RG3_gabriel_regina._t2_tables.get(f"{self._wl}_{self._mode}", {})
        self._t3 = RG3_gabriel_regina._t3_tables.get(f"{self._wl}_{self._mode}", {})
    ...
    def guess(self, history: list[tuple[str, tuple[int, ...]]]) -> str:
        ...
        if turn == 2:
            ...
            if mode == "frequency":
                return _dynamic_best(candidates, vocab, wl, probs)
            if len(candidates) > 6:
                return _dynamic_best(candidates, vocab, wl, probs, max_pool=500, n_probes=15)
